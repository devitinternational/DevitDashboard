...
export const domainApi = {
  create: (payload: DomainPayload) =>
    api.post<{ success: boolean; data: { id: string; slug: string } }>(
      "/domains",
      payload,
    ),
  ...
  createSection: (domainId: string, payload: SectionPayload) =>
    api.post<{ success: boolean; data: { id: string } }>(
      `/domains/${domainId}/sections`,
      payload,
    ),

  createLesson: (domainId: string, sectionId: string, payload: LessonPayload) =>
    api.post<{ success: boolean; data: { id: string } }>(
      `/domains/${domainId}/sections/${sectionId}/lessons`,
      payload,
    ),
  ...
  getVideoUploadUrl: (
    ...
    }>(`/domains/lessons/${lessonId}/upload-url`, { filename, contentType }),

  updateLesson: (
    lessonId: string,
    payload: Partial<LessonPayload> & { videoKey?: string },
  ...
  publish: (domainId: string) =>
    api.patch<{ success: boolean; data: { published: boolean } }>(
      `/domains/${domainId}/publish`,
    ),
};
